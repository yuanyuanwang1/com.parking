package com.wy.sqlcteator.mapping;
...
import com.wy.sqlcteator.mapping.annotations.Fields;

/**
 * @comment 转换对象将Map转换成对象或者将对象转换成另外一个对象。可用于PO到VO的相互转换<br/>
 *          新对象的成员变量名称要与Map的key或者被转换对象的成员变量名称一样。 如果不一样可以通过
 *          com.platform.kit.mapping.annotations.Fields 标注来注释新对象的成员对应那些名称。
 *          可以参看com.platform.kit.mapping.mapping.exam包中的 MapperObj2 和
 *          OldObject_2.<br/>
 * <br/>
 *          使用方式：<br/>
 *          1、public static Mapping mapping = new Mapping();
 *          //将该Mapping注册成为全局变量。<br/>
 *          2、public static Mapping mapping = new Mapping(new
 *          Class[]{MapperObj.class, MapperObj2.class, …………});
...
public class Mapping {
	 ...
	 * @since 1.0.0
	 */
	public <E extends Serializable> E convertMap(Class<E> clazz,
	 ...
	 * @since 1.0.1
	 */
	public <E extends Serializable> List<E> convertListMap(Class<E> clazz,
	 ...
	 * @since 1.0.1
	 */
	public <E extends Serializable> List<E> convertList(Class<E> clazz,
			List<?> objList) {
		...
		Map<String, MappingField> fieldMap = getFieldMap(clazz);
		// 被转换对象的field Map。
		Map<String, MappingField> objFieldMap = getFieldMap(objList.get(0)
				.getClass());
		if (fieldMap == null || objFieldMap == null) {
			return null;
		}
	...
	private <E extends Serializable> E convertObjectFromMap(
			Collection<MappingField> col, Constructor<?> c,
			Map<String, Object> map) {
		if (map == null) {
			return null;
		}
	...
	@SuppressWarnings("unchecked")
	private <E extends Serializable> E convertObjectFromObject(
			Collection<MappingField> col, Constructor<?> c,
